...
// OpenAIQuotaResponse is the public admin quota payload with the local account record ID.
type OpenAIQuotaResponse struct {
	AccountRecordID int64 `json:"account_record_id"`
...
func (h *OpenAIOAuthHandler) QueryQuota(c *gin.Context) {
	...
	response.Success(c, OpenAIQuotaResponse{
		AccountRecordID:  accountID,
		OpenAIQuotaUsage: usage,
	})
}
