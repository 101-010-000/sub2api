...
func (s *GatewayService) initDebugGatewayBodyFile(path string) {
	...
	if info, err := os.Stat(path); err == nil && info.IsDir() { // #nosec G703 -- debug log path is an explicit admin-controlled env setting.
		path = filepath.Join(path, debugGatewayBodyDefaultFilename)
	}

	// 确保父目录存在
	if dir := filepath.Dir(path); dir != "." {
		if err := os.MkdirAll(dir, 0755); err != nil { // #nosec G703 -- debug log directory is an explicit admin-controlled env setting.
			slog.Error("failed to create gateway debug log directory", "dir", dir, "error", err)
			return
		}
	}

	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644) // #nosec G703 -- debug log file is an explicit admin-controlled env setting.
	if err != nil {
