...
func (s *SettingService) GetFeishuConnectOAuthConfig(ctx context.Context) (FeishuConnectOAuthConfig, error) {
	...
	requiredFields := []struct {
		...
		{effective.AppID, "feishu oauth app id not configured"},
		{effective.AppSecret, "feishu oauth app secret not configured"},
		{effective.AuthorizeURL, "feishu oauth authorize url not configured"},
		{effective.TokenURL, "feishu oauth token url not configured"},
		{effective.UserInfoURL, "feishu oauth userinfo url not configured"},
		{effective.RedirectURL, "feishu oauth redirect url not configured"},
		{effective.FrontendRedirectURL, "feishu oauth frontend redirect url not configured"},
	}
	for _, field := range requiredFields {
		if field.value == "" {
			return FeishuConnectOAuthConfig{}, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", field.message)
		}
	}
	urlFields := []struct {
		value   string
		...
		{effective.AuthorizeURL, "feishu oauth authorize url invalid"},
		{effective.TokenURL, "feishu oauth token url invalid"},
		{effective.UserInfoURL, "feishu oauth userinfo url invalid"},
		{effective.RedirectURL, "feishu oauth redirect url invalid"},
	}
	for _, field := range urlFields {
		if err := config.ValidateAbsoluteHTTPURL(field.value); err != nil {
			return FeishuConnectOAuthConfig{}, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", field.message)
		}
	}
	if err := config.ValidateFrontendRedirectURL(effective.FrontendRedirectURL); err != nil {
