...
const (
	...
	defaultGitHubRepo = "TalexDreamSoul/sub2api"
...
type UpdateService struct {
	...
	repo           string // GitHub owner/repo used for update checks and release assets
}

// NewUpdateService creates a new UpdateService
func NewUpdateService(cache UpdateCache, githubClient GitHubReleaseClient, version, buildType string, repoOverride ...string) *UpdateService {
	repo := defaultGitHubRepo
	if len(repoOverride) > 0 {
		if trimmed := strings.TrimSpace(repoOverride[0]); trimmed != "" {
			repo = trimmed
		}
	}
