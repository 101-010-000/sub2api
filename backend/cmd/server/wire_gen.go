...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	feishuUserIdentityRepository := repository.NewFeishuUserIdentityRepository(db)
	feishuNotificationService := service.NewFeishuNotificationService(settingRepository, feishuUserIdentityRepository)
	authHandler := handler.ProvideAuthHandler(configConfig, authService, userService, settingService, promoService, redeemService, totpService, userAttributeService, feishuNotificationService)
	...
	v := repository.ProvideSecretEncryptors(secretEncryptor)
	contentModerationService := service.NewContentModerationService(settingRepository, contentModerationRepository, contentModerationHashCache, groupRepository, userRepository, apiKeyAuthCacheInvalidator, emailService, v...)
	speedService := service.NewSpeedService()
	userHandler := handler.ProvideUserHandler(userService, authService, emailService, emailCache, affiliateService, serviceUserPlatformQuotaRepository, contentModerationService, feishuNotificationService, speedService)
	apiKeyRuntimeCache := repository.NewAPIKeyRuntimeCache(redisClient, configConfig)
	apiKeyRuntimeService := service.NewAPIKeyRuntimeService(apiKeyRuntimeCache)
	apiKeyHandler := handler.ProvideAPIKeyHandler(apiKeyService, apiKeyRuntimeService)
	...
	gatewayService := service.NewGatewayService(accountRepository, groupRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, identityService, httpUpstream, deferredService, claudeTokenProvider, sessionLimitCache, rpmCache, digestSessionStore, settingService, tlsFingerprintProfileService, channelService, modelPricingResolver, balanceNotifyService, serviceUserPlatformQuotaRepository, speedService)
	...
	adminUserHandler := handler.ProvideAdminUserHandler(adminService, concurrencyService, serviceUserPlatformQuotaRepository, billingCache, speedService)
	...
	accountUsageService := service.NewAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, grokQuotaFetcher, openAIQuotaService, balanceNotifyService, usageCache, identityCache, tlsFingerprintProfileService)
	...
	adminSubscriptionHandler := handler.ProvideAdminSubscriptionHandler(subscriptionService, speedService, adminService)
	...
	userAttributeHandler := admin.NewUserAttributeHandler(userAttributeService, adminService)
	...
	adminAPIKeyHandler := admin.ProvideAdminAPIKeyHandler(adminService, apiKeyRuntimeService)
	...
	contentModerationHandler := admin.NewContentModerationHandler(contentModerationService, adminService)
	...
	quotaStatusService := service.NewQuotaStatusService(adminService, settingService, accountUsageService)
	quotaStatusHandler := handler.NewQuotaStatusHandler(quotaStatusService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, quotaStatusHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddlewareWithRuntime(apiKeyService, subscriptionService, apiKeyRuntimeService, configConfig)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, apiKeyService, subscriptionService, apiKeyRuntimeService, opsService, settingService, redisClient)
