...
export async function testEmailNotification(payload: { recipient?: string } = {}): Promise<{ sent: boolean; recipients: string[] }> {
  const { data } = await apiClient.post<{ sent: boolean; recipients: string[] }>('/admin/ops/email-notification/test', payload)
  return data
}
