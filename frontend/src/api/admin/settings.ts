...
export interface DownloadSiteLogoResponse {
  site_logo: string;
}

export async function downloadSiteLogo(
  url: string,
): Promise<DownloadSiteLogoResponse> {
  const { data } = await apiClient.post<DownloadSiteLogoResponse>(
    "/admin/settings/site-logo/download",
    { url },
  );
  return data;
}
...
export async function testFeishuNotification(
  ...
  const { data } = await apiClient.post<{ sent: boolean }>(
    "/admin/settings/feishu-notification/test",
    request,
  );
  return data;
}
